...
import { Flex, Modal, ModalProps, SkeletonInput } from "ant-design-vue";
...
const { latitude, longitude, locationName, isModalOpenMap } = defineProps<
  itemLocationType & { isModalOpenMap: boolean }
>();

const emit = defineEmits<{ toggleModalMap: [] }>();
...
const modalDetailMapProps = computed<ModalProps>(() => ({
  ...
  centered: true,
  footer: null,
  onCancel: () => emit("toggleModalMap"),
}));
</script>

<template>
  <Modal v-bind="modalDetailMapProps" @click="() => emit('toggleModalMap')">
